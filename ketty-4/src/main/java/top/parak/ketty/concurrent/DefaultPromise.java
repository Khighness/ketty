...
public class DefaultPromise<V> implements Promise<V> {
    ...
    private boolean doAwait(long timeNanos, boolean interruptable) throws InterruptedException {
        if (isDone()) {
            return true;
        }
        ...
        long deadline = System.nanoTime() + timeNanos;
        for (;;) {
            synchronized (this) {
                if (isDone()) {
                    logger.info("doAwait done");
                    return true;
                }
                if (System.nanoTime() > deadline) {
                    logger.info("doAwait timeout");
                    return isDone();
                }
                incWaiters();
                try {
                    long waitNanos = (deadline - System.nanoTime()) >> 1;
                    wait(waitNanos / 1000_000, (int) waitNanos % 1000_000);
                    logger.info("doAwait {} ns", waitNanos);
                } finally {
                    decWaiters();
                }
            }
